...
create table public.physio_events (
  id         uuid primary key default gen_random_uuid(),
  user_id    uuid references public.users(id) on delete cascade,
...
-- Indexes
create index idx_keystone_user_date            on public.keystone_actions(user_id,date);
create index idx_physio_user_type_time         on public.physio_events(user_id,type,timestamp desc);
create index idx_callouts_user_time            on public.callouts(user_id,timestamp desc);
create index idx_voice_user_time               on public.voice_transcripts(user_id,timestamp desc);
create index idx_chat_history_user_embedding   on public.chat_history using ivfflat (embedding);
create index idx_chat_history_summaries_embed  on public.chat_history_summaries using ivfflat (embedding);
